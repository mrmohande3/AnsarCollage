...
namespace AnsarCollage.Views.Pages
{
    ...
    public partial class ChaptersPage : ContentPage
    {
        ...
        public ChaptersPage(Cours cours )
        {
            _cours = cours;
            InitializeComponent();
            Initialize();
            nameLable.Text = _cours.course;
        }
        private async void Initialize()
        {
            try
            {
                ...
                var rest = await RestWrapper.Instance.DataRestApi.GetChapter(_cours.id, Basics.Token);
                if (rest.Code == 200)
                {
                    await PopUpService.Instance.PopAsync();
                    mainCollection.ItemsSource = rest.Data.Chapters;
                }
                else
        ...
        private void ChapterItemButton_OnClicked(object sender, EventArgs e)
        {

            if (sender is Grid button && button.BindingContext is Chapter chapter)
                Navigation.PushAsync(new FilesPage(chapter));
        }
